...
name: relentless
description: |
  Long-horizon overnight perfectionist mode (v3). The agent works on a
  ...
  requirements checklist. v3 forces the agent to (a) write the mission
  before writing criteria, (b) generate ≥12 mission-tied qualitative
  criteria scored 0–100 by human-like judgment (not deterministic
  tests), (c) plan + self-review every iteration before touching code
  (DRY / coupling / race / blast-radius / best-practices), (d) display
  iteration numbers prominently and emit a journey-table at the end,
  and (e) only stop when a written Milan Check including a Mission
  alignment row at ≥75/100 and a "smart human comparison" row passes —
  never on "tests pass" or "feels done". Built for "I'm leaving for
  the night, ship something I'd be proud of by morning" sessions —
...
# /relentless — Overnight Perfectionist Mode (v3)
...
direction, until you converge on better scores. The goal is not "all
tests pass" — it is "the qualitative experience of this work, judged
the way a brilliant human would judge it, has reached a real plateau,
AND there is nothing on Milan's mental list of 'could-have-been-better'
that you've left untouched."

The thing that drives gradient descent is the **qualitative criteria**.
They are derived from the **mission**, not from a generic checklist.
They are scored 0–100 by your own judgment looking at the live
output — not by deterministic tests. They are the most important part
of this skill: this is software where the core is non-deterministic
LLM-driven behaviour, so qualitative criteria are how you tell whether
the system is actually *smart*. The deterministic B/E/F dimensions are
the floor; Q is the ceiling you spend the night pushing up.

There is no minimum iteration count, but the empirical signal of a
good run is typically **50–60+ iterations**. Two iterations is not
relentless; that is "implementation done, declared victory."
...
5. **Phase 4** — loop. Each iteration:
   ...
   - **PLAN** — write `iter-NN-plan.md`: what to improve, in which
     way, why this way, alternatives considered, best practices that
     apply, what can go wrong (DRY / coupling / race / blast-radius).
   - **SELF-REVIEW** — review your own plan as a senior eng manager
     would, *non-interactively*. Append a `## Self-review` section to
     the plan file. End with `## Final decision: PROCEED | REVISED |
     ABORT`. The build cannot start until the gate `plan-reviewed`
     passes.
   - **BUILD → COMMIT → MERGE → DEPLOY LIVE → EVALUATE** (run every
     B/E/F observable AND score every Q-criterion 0–100 by judgment)
     **→ JUDGE → write iter-NN.md → run the Milan Check** at the end
     of EVERY iteration (`milan-check-iter-NN.md`) — its verdict
     drives whether the loop continues.
...
# mission-bar-met is required even at iteration level, but only ENFORCED
# (i.e., its non-zero exit forces CONTINUE) when the verdict line says
# PROCEED TO REPORT. This lets early iterations honestly say CONTINUE
# without being blocked, while preventing premature PROCEED.
if grep -qE '^VERDICT:[[:space:]]*PROCEED' "$MILAN_FILE"; then
  if ! "$_LB_BIN/lb-relentless-gate" mission-bar-met "$MILAN_FILE"; then
    echo "FORCED_CONTINUE: mission-bar not yet met; rewriting verdict to CONTINUE LOOP" >&2
    sed -i.bak 's/^VERDICT:.*PROCEED TO REPORT.*/VERDICT: CONTINUE LOOP — mission-bar not met/' "$MILAN_FILE"
  fi
fi
```
...
`mission-bar-met` (only enforced on PROCEED verdicts) requires the
**Mission alignment** row to score ≥75/100 with substantive evidence,
AND the **Smart human comparison** row to list ≥3 specific things a
brilliant human would do that you haven't. Without these, the
sed-rewrite forces the verdict back to CONTINUE LOOP.
...
the lowest-scoring Q-criterion, whichever has more leverage.

If verdict is `PROCEED TO REPORT` and all gates passed: you may exit
Phase 4. Phase 5 will do the canonical Milan Check one more time as
the final gate.

### 4b. Loop exit conditions — saturation against the mission, not completion
...
2. **Q saturation:** the range of Q TOTAL across the last 5
   iterations is ≤ N_CRITERIA (defaults to 12 on the new 0–100
   scale) — i.e., the optimisation has plateaued. Computed by
   `lb-relentless-gate q-saturated`.
...
4. **Mission alignment ≥ 75/100** in the latest Milan Check, with
   substantive evidence. Computed by `lb-relentless-gate
   mission-bar-met`.
5. **Smart-human-comparison row populated** with ≥3 things a brilliant
   human would do that you haven't (also enforced by
   `mission-bar-met`).
...
**The two-iterations test.** If you find yourself considering exit at
iteration 2, 3, 4, or 5 — re-read the user's framing in the TL;DR:
*"An agent that does 2 iterations can't be called to be relentless."*
A small handful of iterations is "implementation done, declared
victory" — not relentless. The exit gate is honest only if the
mission is genuinely realised AND the Q curve has actually plateaued
across 5 iterations. Empirically this lands in the 50–60+ range.
