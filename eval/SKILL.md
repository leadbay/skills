...
description: |
  ...
  a scorecard, and generates an HTML dashboard at `.context/evals/eval-report.html`.
...
4. Writes structured JSON to `.context/evals/` for the dashboard
...
Every eval session result — including sessions run during `--improve` relentless iterations and `--verify` stability rounds — MUST be written to `.context/evals/` as soon as it completes. Do NOT wait until the end of the full run to batch-save. This ensures every iteration appears in the dashboard, not just the final summary.

- Each `/eval` invocation (including those triggered internally during `--improve`) gets its own `RUN_ID` and JSON file written immediately.
- During `--improve` relentless iterations: tag each entry with `"suite": "relentless"`, `"_relentless_iter": N`, and `"_relentless_goal": "..."` so the dashboard can group and label them.
- During `--verify` stability rounds: tag entries with `"suite": "verify"` and `"_verify_round": N`.
- A plain `/eval` run uses `"suite": "eval"` (unchanged).
