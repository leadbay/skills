...
**DO NOT use the Skill tool to invoke relentless.** Calling `Skill({ skill: 'relentless' })` only loads the skill content — it does not execute it. You must execute relentless inline by running its phases directly in this same conversation turn.

Execute the relentless loop now with this seeded mission:
