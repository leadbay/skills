...
Every eval session result (including those run during `--improve` relentless iterations and `--verify` stability rounds) MUST be written to `.context/evals/` as soon as it completes. Do NOT wait until the end of the full run to batch-save. This ensures every iteration appears in the dashboard, not just the final summary.

- Each `/eval` invocation (including those triggered internally during `--improve`) gets its own `RUN_ID` and JSON file.
- During `--improve` relentless iterations: tag each entry with `"suite": "relentless"` and add `"_relentless_iter": N` and `"_relentless_goal": "..."` fields so the dashboard can group them.
...
- Test: run `/eval --workflow [N]` (via the eval skill phases 0-7, NOT with --improve) after each build — **each test run MUST write its result to `.context/evals/` (Phase 6 logging rule)**; tag with `suite: "relentless"`, `_relentless_iter: N`, `_relentless_goal: "..."` so the dashboard tracks every iteration
