...
jq '[.entries[] | select(.evidence.judge_scores.mission_match < 5 and .passed == true)]' "$LATEST"
```

Three cases:

**Case 1 — all MM == 5 (or all passed == false due to skill error):**
Print:
```
✓ All workflows at MM:5. No self-improvement needed.
```
Stop. Do not invoke relentless.

**Case 2 — one or more workflows have MM < 5 AND passed == true (real product signal, not skill error):**
...
- `current_mm` — from `evidence.judge_scores.mission_match`
- `failing_criteria` — from `[evidence.per_criterion[] | select(.pass == false) | .criterion]`
...
```
MM:[N]/5 on [workflow_name] — launching self-improvement loop.
Failing criteria:
  [✗] "criterion text"
...
```
--feature "Improve [workflow_name] (prompt: [prompt_name]) from MM:[current_mm] to MM:5.
...
Read the latest result:
  ...
  jq '.entries[0].evidence.judge_scores.mission_match' $LATEST   # target: 5
  jq '[.entries[0].evidence.per_criterion[] | select(.pass==false)]' $LATEST  # target: []

Regression guard: invoke /eval skill with '--workflow [passing_workflow_ids]' once MM:5 is reached.
All must still pass (passed:true). If any regress, revert the last template edit and try a different approach.

Exit condition: MM:5 stable across 2 consecutive /eval runs AND a fresh-context second-opinion judge
