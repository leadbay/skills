...
In practice: if you would naturally start the issue with "we should
build X" and you ARE part of the team that builds it, this isn't
your skill. If you would naturally start with "the user / a customer
hit X", it is.
...
it. If the reporter insists on a fix idea, accept it as ONE sentence
labelled `Reporter hunch:` at the very end — never in the title,
never in the body proper, never as a section.
...
What is **not** this sin: naming a workflow gap from the
reporter's own dogfooding session. *"No MCP path converts
manually-sourced name+linkedin+title rows into outreach-ready
emails — forces a parallel Apollo/Hunter pass"* (#3676) names a
gap the dogfooder hit and what they did next. Not a fix, not a
customer-wide claim. Keep it.
...
split. The CTO replied to `#3461`'s wall: *"this issue format is
not useful because it jumps to solutions without clearly defining
the actual problems."*
...
- **Register honest:** if the source said "a bit confusing", the
  ...
  filter `Industrie` confusing"* — or, if you suspect they gave up
  but can't prove it from the source: *"(inferred: rep may have
  abandoned the segment — confirm with $REPORTER)"*. The
  difference: an honest issue costs nothing in priority; an inflated
  one costs credibility on every issue you file next.
...
If you find yourself writing five `user stories` under one issue,
**stop**. That's not one issue — it's five. File them separately.
(Real anti-pattern: `#3630` MCP Wow Effect, 6255 chars of speculative
user stories. The CTO replied: *"this issue format is not useful
...
Quickly check: is this a **reporter-to-team issue** (user pain,
customer complaint, workflow gap for the team to act on), or an
**owner-internal backlog note** (FE/BE ask, internal TODO, work-
tracking goal where the team owns the fix)?

If owner-internal: exit. Tell the reporter "this looks like
owner-internal team backlog — write it however suits, the team
decides their own work." Don't run the gates.

If reporter-to-team: continue.
...
- Feature: `<who> can't <do what>` → "Actis can't tell Leadbay filters from CRM fields"
...
- *"But I already wrote the solution, I just want to save the engineer's time."* — You're not saving time, you're anchoring the engineer on the wrong problem. Cut the solution. If you have a strong hunch, keep it as ONE `Reporter hunch:` sentence at the bottom.
