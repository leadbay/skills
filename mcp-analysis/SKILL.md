...
the optional `--improve` flag (off by default).
...
DAYS="${ARG_DAYS:-30}"
POSTHOG_HOST=https://eu.posthog.com
# IMPROVE=1 when the invocation contains --improve, else 0.
echo "window: last ${DAYS}d  ·  user filter: ${ARG_USER:-<all>}  ·  improve: ${IMPROVE:-0}"
```

`--improve` does NOT change Phases 1–6 — the analysis + archive run identically.
It only activates Phase 7 (the fix loop) AFTER the report is complete and
archived. A run without `--improve` is read-only as always.
...
## Phase 7 — Improve loop (ONLY when `--improve` is set)

If `IMPROVE` is not set, STOP after Phase 6 — the run is complete. Everything
below runs ONLY with `--improve`.
...
**Phase 7 Gate — emit this block to close an --improve run:**
